...
To accomplish intra-cluster homogeneity analysis, we employed two computational methods. The first method identified the significant difference between biological pathways of drug targets’ protein targets at each cluster, while the second evaluated drug chemical structure similarity at each cluster. 
...
#  ylab("Dice similarity of SMILEs") +
 ...
 # coord_flip()
#######
my_comparisons <- list(c('Cluster 2','Intercluster'),c('Cluster 1','Intercluster'))
ggp_value <- ggboxplot(dat,x = 'Cluster',y = 'sim',fill = 'Cluster', col = "gray40",alpha=0.5)+
...
inset.plot <- ggplot(drug_mem_eff_tox %>% filter(efficacy <=15 & tox <=15), aes(x = efficacy, y = tox))+
  geom_density_2d_filled(alpha = 0.5) +
  geom_density_2d(size = 0.25, colour = "black")+
  geom_point(aes(x = efficacy, y = tox, col = -log(pval), shape = factor(member)),size  =2.5)+
  ...
  guides(shape = guide_legend("Cluster"), fill = "none", color = "none") +
...
#save 8x6 pdf
tox_eff_plot <- ggplot(drug_mem_eff_tox, aes(x = efficacy, y = tox))+
  geom_density_2d_filled(alpha = 0.5) +
  geom_density_2d(size = 0.25, colour = "black")+
  geom_point(aes(x = efficacy, y = tox, col = -log(pval), shape = member),size  =3)+
...
ggarrange(ggp_value,NULL,plot.with.inset,
          ncol =1, nrow = 3,  align = "hv", 
          widths = c(2, 0.1,1), heights = c(1,0.05, 2),
          common.legend = TRUE,labels = c("A", "", "B"),
          label.x = c(0.03),  # X-coordinate for labels
          label.y = c(1.05) )
# save 8|12
```
...
```{r}
Final_RTG <- read_excel("Documents/Submitted_paper/oncogenisis/Final_RTG.xlsx")
Final_RTG$Drug[Final_RTG$Drug=='LS']='LY30-Sapa'
Final_RTG_ls_kg <- Final_RTG %>% dplyr::filter(Drug %in% c("LY30-Sapa","Sapanisertib","LY3009120") & `Drug_concentration(uM)`== 1 & 
                                                 Cell_line %in% c("KG-1"))

Final_RTG_ls_kg_ls <- Final_RTG_ls_kg %>% dplyr::filter(Drug %in% c("LY30-Sapa"))
Final_RTG_ls_kg_Sap <- Final_RTG_ls_kg %>% dplyr::filter(Drug %in% c("Sapanisertib"))
Final_RTG_ls_kg_LY <- Final_RTG_ls_kg %>% dplyr::filter(Drug %in% c("LY3009120"))

Final_RTG_ls_kg$Drug=factor(Final_RTG_ls_kg$Drug,levels = c("Sapanisertib","LY3009120","LY30-Sapa"))
my_comparisons1 <- list( c("Sapanisertib", "LY30-Sapa") )
p1=ggboxplot(Final_RTG_ls_kg,x = 'Drug',y = 'Cell_viability',fill = 'Drug',outlier.shape = NA)+
  stat_compare_means(comparisons = my_comparisons1)+
  xlab('')+ggtitle('KG-1')+
  scale_fill_manual(values = c('skyblue','yellow','green'))+
...
#-------------HNT-----------
Final_RTG_ls_hnt <- Final_RTG %>% dplyr::filter(Drug %in% c("LY30-Sapa","Sapanisertib","LY3009120") & `Drug_concentration(uM)`== 1 & 
                                                 Cell_line %in% c("HNT"))
Final_RTG_ls_hnt_ls <- Final_RTG_ls_hnt %>% dplyr::filter(Drug %in% c("LY30-Sapa"))
Final_RTG_ls_hnt_Sap <- Final_RTG_ls_hnt %>% dplyr::filter(Drug %in% c("Sapanisertib"))
Final_RTG_ls_hnt_LY <- Final_RTG_ls_hnt %>% dplyr::filter(Drug %in% c("LY3009120"))


Final_RTG_ls_hnt$Drug=factor(Final_RTG_ls_hnt$Drug,levels = c("Sapanisertib","LY3009120","LY30-Sapa"))
my_comparisons2 <- list( c("LY3009120", "LY30-Sapa") )
p2=ggboxplot(Final_RTG_ls_hnt,x = 'Drug',y = 'Cell_viability',fill = 'Drug',outlier.shape = NA)+xlab('')+ylim(0,110)+
  stat_compare_means(comparisons = my_comparisons2,label.y = 90)+
  theme_bw(base_line_size = .1)+ggtitle('HNT')+
  scale_fill_manual(values = c('skyblue','yellow','green'))+
  theme(legend.position = 'none',
...
#-------------Nomo-----------
Final_RTG_ls_nomo <- Final_RTG %>% dplyr::filter(Drug %in% c("LY30-Sapa","Sapanisertib","LY3009120") & `Drug_concentration(uM)`== 1 & 
                                                 Cell_line %in% c("NOMO"))
Final_RTG_ls_nomo_ls <- Final_RTG_ls_nomo %>% dplyr::filter(Drug %in% c("LY30-Sapa"))
Final_RTG_ls_nomo_Sap <- Final_RTG_ls_nomo %>% dplyr::filter(Drug %in% c("Sapanisertib"))
Final_RTG_ls_nomo_LY <- Final_RTG_ls_nomo %>% dplyr::filter(Drug %in% c("LY3009120"))


Final_RTG_ls_nomo$Drug=factor(Final_RTG_ls_nomo$Drug,levels = c("Sapanisertib","LY3009120","LY30-Sapa"))
my_comparisons3 <- list( c("LY3009120", "LY30-Sapa") , c("Sapanisertib", "LY30-Sapa"))
p3=ggboxplot(Final_RTG_ls_nomo,x = 'Drug',y = 'Cell_viability',fill = 'Drug',outlier.shape = NA)+xlab('')+
  stat_compare_means(comparisons = my_comparisons3,label.y = c(55,78))+
  theme_bw(base_line_size = .1)+ggtitle('NOMO')+ylim(0,113)+
  scale_fill_manual(values = c('skyblue','yellow','green'))+
  theme(legend.position = 'none',
        axis.title = element_text(size = 14,face = 'bold'),
        axis.text = element_text(size = 12,face = 'bold'),
...
#-------------ML2-----------
Final_RTG_ls_hl60 <- Final_RTG %>% dplyr::filter(Drug %in% c("LY30-Sapa","Sapanisertib","LY3009120") & `Drug_concentration(uM)`== 1 & 
                                                 Cell_line %in% c("HL60"))

Final_RTG_ls_hl_ls <- Final_RTG_ls_hl60 %>% dplyr::filter(Drug %in% c("LY30-Sapa"))
Final_RTG_ls_hl_Sap <- Final_RTG_ls_hl60 %>% dplyr::filter(Drug %in% c("Sapanisertib"))
Final_RTG_ls_hl_LY <- Final_RTG_ls_hl60 %>% dplyr::filter(Drug %in% c("LY3009120"))
wilcox.test(Final_RTG_ls_hl_ls$Cell_viability,Final_RTG_ls_hl_Sap$Cell_viability)


Final_RTG_ls_hl60$Drug=factor(Final_RTG_ls_hl60$Drug,levels = c("Sapanisertib","LY3009120","LY30-Sapa"))
my_comparisons4 <- my_comparisons3
p4=ggboxplot(Final_RTG_ls_hl60,x = 'Drug',y = 'Cell_viability',fill = 'Drug',outlier.shape = NA)+xlab('')+
  stat_compare_means(comparisons = my_comparisons4,label.y = c(70,90))+
  theme_bw(base_line_size = .1)+ggtitle('HL60')+ylim(0,110)+
  scale_fill_manual(values = c('skyblue','yellow','green'))+
  theme(legend.position = 'none',
        axis.title.y = element_blank(),
        axis.title = element_text(size = 14,face = 'bold'),
